...
N=${1:-10}
for ((i=0; i<$N; i++)); do 
	curl ${SERVICE_INGRESS_HOST}/sentence/async --connect-to ${SERVICE_INGRESS_HOST}:80:${SERVICE_HOST}:${INGRESS_PORT} 
	echo "" ; 
	# oppure 
#	curl ${SERVICE_HOST}/sentence/async --header "Host: ${SERVICE_INGRESS_HOST}" 
#	echo "" ; 
done 
